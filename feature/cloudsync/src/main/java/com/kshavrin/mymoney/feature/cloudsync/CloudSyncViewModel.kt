...
class CloudSyncViewModel @Inject constructor(
    ...
    private val _actions = MutableSharedFlow<CloudSyncAction>(
    ...
    fun onEvent(event: CloudSyncEvent) {
        when (event) {
            ...
            CloudSyncEvent.BackClicked -> viewModelScope.launch {
    ...
    private fun applyOutcome(target: SyncTarget, outcome: SyncOutcome) {
        when (outcome) {
            is SyncOutcome.ConflictDetected -> _state.value = _state.value.copy(
            ...
            SyncOutcome.Pushed,
            SyncOutcome.Pulled,
            SyncOutcome.UpToDate -> refresh(target)
        }
    }
    ...
    private fun card(target: SyncTarget): TargetCardState = when (target) {
        SyncTarget.Dropbox -> _state.value.dropbox
        SyncTarget.GoogleDrive -> _state.value.drive
    }

    private fun updateCard(target: SyncTarget, transform: (TargetCardState) -> TargetCardState) {
    ...
    @StringRes
    internal fun mapError(error: SyncError): Int = when (error) {
