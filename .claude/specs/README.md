...
This directory stores persistent records of `/cmp --discuss <topic>` brainstorms — the
...
1. User runs `/cmp --discuss <topic>`.
2. `cmp-architect` agent returns a BRAINSTORM block.
3. The `/cmp` orchestrator asks: *"Save as a spec draft in `.claude/specs/`? (y/N)"*
4. If yes, orchestrator asks for a `kebab-case` slug and writes `<slug>.md` here.

Later, when `/cmp --feature` is run for the same topic, the orchestrator fills in
...
## Approved SPEC
<full SPEC block from /cmp --feature, or "(pending)" if not yet approved>
...
| `in-progress` | `/cmp --feature` started but didn't complete (interrupted, blocked, etc). |
