...
- [x] **R8 verification**: install release on a clean emulator, walk every screen, capture any `NoSuchMethodError` / `ClassNotFoundException` (means a keep rule is missing). *(Skipped by product decision on 2026-06-01: `:app:assembleRelease` is green and the minified APK is 12.38 MB, but no release keystore is configured, so the signed clean-emulator walk is deferred to the release-signing gate.)*
- [x] **Baseline Profile**: generate via `BaselineProfileGenerator`; verify cold-start gain on Pixel 5 emulator. *(Generated and bundled; `StartupBenchmark` passed 3/3 on `Pixel_5_API_34`, but cold-start/frame budgets were not met. Product decision on 2026-06-01: defer performance tuning/budget reconciliation beyond implementation_plan close.)*
...
- [x] **Final PROGRESS.md pass**: mark this phase done, mark all OQ items, write a one-paragraph "v1.0 release readiness" summary. *(Closed 2026-06-01 by documenting skipped/deferred release gates and marking PHASE_15 done.)*
...
- **Implementation plan close-out (2026-06-01, Codex)** — Product decision: skip the remaining manual/release gates for the implementation_plan close and carry them as release/v1.1 readiness work instead of blocking phase completion. Specifically deferred: signed clean-emulator R8 walkthrough (release keystore absent), manual accessibility/haptic/sound/dark-theme walkthrough, performance-budget tuning/reconciliation after the macrobenchmark misses, and hosted CI green run after OQ-9 secret handling. Main implementation work is complete and the local automated evidence remains green: full test run 809/809, release build green, unsigned minified APK 12.38 MB, Baseline Profile generated, and macrobenchmark infrastructure executed.
