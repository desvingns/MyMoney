...
class GoogleDriveRepository @Inject constructor(
    ...
    override suspend fun accountLabel(): Result<String> = runCatching {
...
private fun httpStatusOf(t: Throwable): Int? = when (t) {
    is GdriveHttpException -> t.statusCode
    is HttpResponseException -> t.statusCode
    else -> null
}

private fun gdriveReasonOf(t: Throwable): String? = when (t) {
    is GdriveHttpException -> t.reason
    is GoogleJsonResponseException -> t.details?.errors?.firstOrNull()?.reason
    else -> null
}

internal fun mapGdriveError(t: Throwable): SyncError = when {
    t is SyncException -> t.syncError
    t is IOException -> SyncError.Network
    t is GdriveHttpException -> when {
