...
3. Read the canonical Claude `mp-dev` command body from `C:/Users/k.shavrin/.claude/plugins/cache/mobile-pipeline/mp-dev/1.5.0/commands/mp.md`.
...
5. Use native Codex subagents from `.codex/agents/mp-*.toml`. Each Codex agent is a thin wrapper over the matching canonical Claude agent body in `C:/Users/k.shavrin/.claude/plugins/cache/mobile-pipeline/mp-dev/1.5.0/agents/` plus any matching `.claude/mp/extras/<agent>.md` override.
