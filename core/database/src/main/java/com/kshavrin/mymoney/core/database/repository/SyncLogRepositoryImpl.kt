...
class SyncLogRepositoryImpl @Inject constructor(
    ...
    override suspend fun insert(entry: SyncLogEntry): Long = withContext(ioDispatcher) {
        dao.insert(entry.toEntity())
    }

    override suspend fun recentByTarget(target: String, limit: Int): List<SyncLogEntry> = withContext(ioDispatcher) {
        dao.recentByTarget(target, limit).map { it.toDomain() }
    }

    override suspend fun pruneOld(target: String) = withContext(ioDispatcher) {
        dao.pruneOld(target)
    }
}
