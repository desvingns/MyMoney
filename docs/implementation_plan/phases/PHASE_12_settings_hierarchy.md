...
- [ ] Re-read TDD anchors.
- [ ] Theme selection — write `AppSettings.themeMode` via `AppSettingsRepository.update`. The root `MyMoneyTheme(themeMode = ...)` should accept the mode as a parameter (extend the API from PHASE_03 if needed) and choose dark/light/system accordingly. `MainActivity` observes settings → recomposes theme.
- [ ] Language selection — add `androidx.appcompat:appcompat` dep (only thing needed for `AppCompatDelegate.setApplicationLocales`; per-app language uses this API on Android 13+ and falls back to `LocaleListCompat` on older devices). Switch happens live (the activity recreates).
- [ ] Add `locales_config.xml` + `android:localeConfig` on `<application>`. Verify: install on Android 14 emulator → System Settings → App info → Language → MyMoney shows the locale switcher chip.
- [ ] Add `privacy_policy_<lang>.html` + `help_<lang>.html` placeholders. Open in `WebView` with `settings.javaScriptEnabled = false` per §9.6.
- [ ] Add Gradle OSS licenses plugin (`id("com.google.android.gms.oss-licenses-plugin")` in `:app/build.gradle.kts`). On `licenses` entry click → `startActivity(Intent(context, OssLicensesMenuActivity::class.java))`.
- [ ] **S18 Backup**: Export — open SAF `CreateDocument("application/x-sqlite3")`, write contents of `/data/data/.../databases/monefy.db` to the chosen Uri. Import — open SAF `OpenDocument("application/x-sqlite3")`, read into temp file, close Room DB (`MoneyDatabase.close()`), copy temp file over `monefy.db`, re-open Room. Catch `IOException` and `SQLiteException`; show errorBanner. On success — toast "Restored, restart app" and call `Process.killProcess(Process.myPid())` (force restart so new DB is read).
- [ ] **OQ-8 local rotation**: after each export, list backups in the SAF folder (by parent Uri walk), sort by date, delete all beyond the 3 newest.
- [ ] Routes added to `MyMoneyNavHost`: `settings_root`, `settings_theme`, `settings_language`, `settings_about`, `settings_about_privacy`, `settings_about_help`, `settings_backup`. Right-drawer "Settings" → `settings_root`.
- [ ] Live test:
