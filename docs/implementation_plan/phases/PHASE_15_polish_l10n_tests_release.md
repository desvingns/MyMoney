...
- **Device coverage slice S03 initial controls (2026-05-27)** — Added `7d70c4d` and `35fa5df` for direct S03 coverage: Back dispatches its event and Save is disabled in the initial invalid transfer state. Scoped `TransferScreenUiTest` passed **2/2, 0 failed, 0 skipped** on `Pixel_5_API_34`; the host-AVD helper completed its mandatory 60-second wait after each automated attempt. TDD section 4.8 lines 737-744 requires the keypad to be hidden until the amount field is focused, while `TransferScreen` currently renders it unconditionally, so keypad coverage requires a production bugfix before it can be tested honestly; AS-6/AS-7 remain E2E work.
