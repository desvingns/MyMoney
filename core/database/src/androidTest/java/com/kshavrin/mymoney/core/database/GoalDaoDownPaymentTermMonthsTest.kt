...
class GoalDaoDownPaymentTermMonthsTest {
    ...
    private fun creditGoalEntity(name: String = "Mortgage") = GoalEntity(
        name = name,
    ...
    private fun savingsGoalEntity(name: String = "Car") = GoalEntity(
        name = name,
    ...
    @Test
    fun credit_goal_downPayment_survives_round_trip_via_findById() = runTest {
    ...
    @Test
    fun credit_goal_termMonths_survives_round_trip_via_findById() = runTest {
        val id = db.goalDao().upsert(creditGoalEntity())
        val read = db.goalDao().findById(id)
    ...
    @Test
    fun credit_goal_annualRatePercent_survives_round_trip_via_findById() = runTest {
        val id = db.goalDao().upsert(creditGoalEntity())
        val read = db.goalDao().findById(id)
    ...
    @Test
    fun credit_goal_downPayment_and_termMonths_survive_round_trip_via_observeActive() = runTest {
    ...
    @Test
    fun credit_goal_upsert_update_overwrites_downPayment_and_termMonths() = runTest {
        val id = db.goalDao().upsert(creditGoalEntity())
        db.goalDao().upsert(
    ...
    @Test
    fun credit_goal_termDate_is_not_written_by_toEntity_and_reads_back_as_null() = runTest {
    ...
    @Test
    fun savings_goal_downPayment_is_null_after_round_trip() = runTest {
    ...
    @Test
    fun savings_goal_termMonths_is_null_after_round_trip() = runTest {
        val id = db.goalDao().upsert(savingsGoalEntity())
        val read = db.goalDao().findById(id)
    ...
    @Test
    fun savings_goal_targetAmount_survives_round_trip_as_BigDecimal() = runTest {
        val id = db.goalDao().upsert(savingsGoalEntity())
        val read = db.goalDao().findById(id)
    ...
    @Test
    fun savings_and_credit_goals_coexist_and_are_distinguishable_by_variant() = runTest {
