...
class GenerateDueRecurringUseCaseTest {
    ...
    private val useCase = GenerateDueRecurringUseCase(
    ...
    @Test
    fun `daily template due one hour ago generates exactly one new transaction`() = runTest {
    ...
    @Test
    fun `generated occurrence is a brand new row at the original due instant`() = runTest {
        val dueInstant = now.minus(1, ChronoUnit.HOURS)
        templateRepo.seed(dailyTemplate(nextRunAt = dueInstant))
    ...
    @Test
    fun `generated occurrence copies the template fields`() = runTest {
    ...
    @Test
    fun `single due daily template advances nextRunAt past now`() = runTest {
        val template = dailyTemplate(nextRunAt = now.minus(1, ChronoUnit.HOURS))
        templateRepo.seed(template)
    ...
    @Test
    fun `single due daily template advances nextRunAt to the scheduler value`() = runTest {
        val template = dailyTemplate(nextRunAt = now.minus(1, ChronoUnit.HOURS))
        templateRepo.seed(template)
    ...
    @Test
    fun `template behind by several periods catches up one occurrence per missed run`() = runTest {
    ...
    @Test
    fun `catch-up leaves nextRunAt strictly after now`() = runTest {
        val template = dailyTemplate(nextRunAt = now.minus(3, ChronoUnit.DAYS))
        templateRepo.seed(template)
    ...
    @Test
    fun `template is deactivated when the next run would pass endsAt`() = runTest {
    ...
    @Test
    fun `deactivation generates the in-range occurrence and skips updateNextRun`() = runTest {
        val template = dailyTemplate(
            nextRunAt = now.minus(1, ChronoUnit.DAYS),
            endsAt = now.minus(1, ChronoUnit.HOURS),
        )
    ...
    @Test
    fun `no due templates produces no writes`() = runTest {
    ...
    @Test
    fun `inactive due template is skipped`() = runTest {
    ...
    @Test
    fun `processing a due template touches only the template and transaction repositories`() = runTest {
        val template = dailyTemplate(nextRunAt = now.minus(1, ChronoUnit.HOURS))
        templateRepo.seed(template)
