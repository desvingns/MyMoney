...
.\gradlew.bat --no-daemon --continue `
...
For a visual review, load the pulled PNG with the local image viewer tool. If
`adb devices -l` is empty, first confirm that `Pixel_5_API_34` is booted on the
primary Windows host, then repeat the guest `adb kill-server` / `adb start-server`
sequence.
