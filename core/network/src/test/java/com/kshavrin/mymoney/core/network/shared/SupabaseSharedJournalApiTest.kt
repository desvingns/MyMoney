...
class SupabaseSharedJournalApiTest {
    ...
    @Test
    fun `push returns same domain operation on idempotent retry`() = runTest {
    ...
    @Test
    fun `push maps dto fields to domain operation correctly`() = runTest {
        val dto = stubOpDto(
            id = "op-abc",
            workspaceId = "ws-2",
    ...
    @Test
    fun `push converts entity kind to correct rpc string`() = runTest {
    ...
    @Test
    fun `push parses json string payload and passes it to rpc`() = runTest {
    ...
    @Test
    fun `push with null payload passes null to rpc`() = runTest {
        fakeRpc.pushOperationResult = Result.success(stubOpDto())
    ...
    @Test
    fun `push propagates rpc failure as Result failure`() = runTest {
    ...
    @Test
    fun `pull maps dto list to domain operations preserving order`() = runTest {
        val dtos = listOf(
    ...
    @Test
    fun `pull passes workspace id after sequence and limit to rpc`() = runTest {
    ...
    @Test
    fun `pull returns failure when rpc fails`() = runTest {
    ...
    @Test
    fun `pull returns failure when dto contains unknown entity kind`() = runTest {
    ...
    @Test
    fun `listPendingConflicts maps pending conflict dto to domain correctly`() = runTest {
        val dto = stubConflictDto(
            id = "conflict-42",
            entityKind = "account",
            entityId = "acct-1",
    ...
    @Test
    fun `listPendingConflicts maps resolved conflict with resolver and resolution time`() = runTest {
    ...
    @Test
    fun `listPendingConflicts passes workspace id to rpc`() = runTest {
    ...
    @Test
    fun `listPendingConflicts returns failure when rpc fails`() = runTest {
    ...
    @Test
    fun `listPendingConflicts returns failure when conflict dto has unknown entity kind`() = runTest {
    ...
    @Test
    fun `listPendingConflicts returns failure when conflict dto has unknown status`() = runTest {
    ...
    @Test
    fun `resolveConflict maps superseding operation dto to domain correctly`() = runTest {
        val supersedingDto = stubOpDto(
            id = "op-resolution",
            serverSequence = 10L,
            entityKind = "category",
        )
        ...
        val result = api.resolveConflict(
    ...
    @Test
    fun `resolveConflict forwards conflict id and winner operation id to rpc`() = runTest {
    ...
    @Test
    fun `resolveConflict propagates rpc failure`() = runTest {
    ...
    @Test
    fun `resolveConflict returns failure when returned dto has unknown entity kind`() = runTest {
