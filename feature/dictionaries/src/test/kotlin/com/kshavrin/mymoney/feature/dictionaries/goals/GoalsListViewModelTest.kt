...
class GoalsListViewModelTest {
    ...
    @Test
    fun `seeded active goals populate rows`() =
        runTest {
    ...
    @Test
    fun `archived goals are excluded from rows`() =
        runTest {
            fakeRepo.seed(
                listOf(
                    goal(1L, "Active"),
    ...
    @Test
    fun `ItemClicked emits NavigateEdit action with the correct id`() =
        runTest {
    ...
    @Test
    fun `rows update when repository emits a new list`() =
        runTest {
            viewModel.state.test {
                awaitItem()
    ...
    @Test
    fun `empty repository keeps rows empty`() =
        runTest {
