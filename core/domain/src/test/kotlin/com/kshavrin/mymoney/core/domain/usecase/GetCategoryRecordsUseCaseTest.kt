...
class GetCategoryRecordsUseCaseTest {
    ...
    @Test
    fun `preserves SQL total-desc order from the grouped query`() =
        runTest {
    ...
    @Test
    fun `maps header fields straight from the grouped query`() =
        runTest {
            seedAccountAndCurrency()
            transactionRepo.seedCategoryGroups(
                group(
    ...
    @Test
    fun `wraps the SQL total in Money with the account currency`() =
        runTest {
            seedAccountAndCurrency()
            transactionRepo.seedCategoryGroups(
                group(categoryId = 10L, name = "Bills", total = BigDecimal("50.00"), count = 1),
    ...
    @Test
    fun `buckets period transactions under their category group`() =
        runTest {
            seedAccountAndCurrency()
            transactionRepo.seedCategoryGroups(
                group(categoryId = 10L, name = "Bills", total = BigDecimal("50.00"), count = 1),
    ...
    @Test
    fun `orders bucketed transactions by occurredAt descending`() =
        runTest {
            seedAccountAndCurrency()
            transactionRepo.seedCategoryGroups(
    ...
    @Test
    fun `returns empty list when the grouped query is empty`() =
        runTest {
            seedAccountAndCurrency()
            transactionRepo.seedCategoryGroups()
            transactionRepo.seedPeriodTransactions(
    ...
    @Test
    fun `category filter returns only the matching category group and transactions`() =
        runTest {
            seedAccountAndCurrency()
            transactionRepo.seedCategoryGroups(
                group(categoryId = 10L, name = "Bills", total = BigDecimal("50.00"), count = 1),
    ...
    @Test
    fun `groups with no matching transactions get an empty bucket`() =
        runTest {
            seedAccountAndCurrency()
            transactionRepo.seedCategoryGroups(
                group(categoryId = 10L, name = "Bills", total = BigDecimal("50.00"), count = 1),
    ...
    @Test
    fun `forAccounts aggregates same-currency groups and excludes archived accounts`() =
        runTest {
    ...
    @Test
    fun `forAccounts category filter keeps only the requested aggregate category`() =
        runTest {
            val cardAccount = account(id = 2L, name = "Card", currencyId = currencyId)
