...
- **Tests — UI** — IN PROGRESS. Added compilable instrumentation coverage for `MonefyDonutChart` accessibility semantics, AS-12 `PeriodStrip` custom-range entry, and S12 swipe-delete affordance. `:app:assembleDebugAndroidTest` and `:core:designsystem:assembleDebugAndroidTest` pass. No AVD/device is installed on this host, so these tests were not executed and the `createAndroidComposeRule<MainActivity>()` onboarding -> dashboard -> add expense critical path remains open.
