...
- Impl commit: `4a6eda4` — feat: empty-state donut (gray ring + expense-category icons + 0/0 center)
- Test commit: `259d977` — test: cover empty-state donut ring + evenAngles
- Changed files (production):
  - `core/designsystem/.../donut/DonutGeometry.kt` — pure `evenAngles(count)` (start -90°, step 360/count)
  - `core/designsystem/.../donut/MonefyDonutChart.kt` — `emptyStateIcons` param + unwired `onEmptyCategoryClick`; shared `drawCategoryIcon` helper; empty-state gray `outline` ring + full-colour icons + SPEC-02 two-line 0/0 center
  - `feature/dashboard/.../DashboardState.kt` — `expenseCategoryPlaceholders`
  - `feature/dashboard/.../DashboardViewModel.kt` — injected `CategoryRepository`; observes `observeByKind(Expense)`, sorts by `sortOrder`, builds fraction=0 placeholder slices
  - `feature/dashboard/.../DashboardScreen.kt` — passes `emptyStateIcons`
- Tests: `DonutGeometryTest` (evenAngles), `MonefyDonutChartUiTest` (instrumented, empty-state), `DashboardViewModelTest` (placeholder build/sort/colour/exclude-income)
- Verification: `:core:designsystem` + `:feature:dashboard` JVM 92 passed / 0 failed, lint ok; Reviewer pass; Verifier hilt_graph=ok. Instrumented `MonefyDonutChartUiTest` not run (no device) — deferred to a device pass.
- NOT pushed (local-only per PROGRESS Decision 2).
