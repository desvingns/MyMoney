...
The authoritative spec is `TDD/MyMoney/MyMoney_TDD.md`. This directory is the **how-to**, not the **what** — every `PHASE_NN_*.md` points back to the TDD sections that contain the source of truth.
...
3. **Re-read the TDD anchors** cited in that file (line ranges to `TDD/MyMoney/MyMoney_TDD.md`). Do not read the whole TDD — the line ranges are precise on purpose.
...
| Token | Meaning |
|---|---|
| **TDD** | `TDD/MyMoney/MyMoney_TDD.md` — the authoritative spec. |
