...
|------|---------|----------------|
| `STATE.md` | Live state — what's happening *now* | **Always** after each `/cmp` run |
...
`STATE.md` reflects *current* state. Update it after **every** `/cmp` run, even if `DOCUMENTATION.md` and `CLAUDE.md` need no changes.
...
| **Now → Last completed** | One line: iteration label, what shipped, commit hash, date. Example: `Iteration 6 sub-C — cmp-architect agent (commit abc1234, 2026-05-19)`. |
