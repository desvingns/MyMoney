...
class RoundTripTest {
    ...
    @Test
    fun currency_roundtrip() = runTest {
        val id = db.currencyDao().upsert(
    ...
    @Test
    fun category_roundtrip() = runTest {
    ...
    @Test
    fun transaction_roundtrip() = runTest {
        val currencyId = db.currencyDao().upsert(
            CurrencyEntity(code = "USD", symbol = "$", name = "US Dollar", decimalDigits = 2, isActive = true, sortOrder = 0),
        )
    ...
    @Test
    fun recurring_template_roundtrip() = runTest {
        val currencyId = db.currencyDao().upsert(
            CurrencyEntity(code = "USD", symbol = "$", name = "US Dollar", decimalDigits = 2, isActive = true, sortOrder = 0),
        )
        val accountId = db.accountDao().upsert(
        ...
        val id = db.recurringTemplateDao().upsert(
    ...
    @Test
    fun sync_log_roundtrip() = runTest {
        val id = db.syncLogDao().insert(
            SyncLogEntity(
                target = "dropbox",
                event = "push",
                entityKind = null,
    ...
    @Test
    fun search_history_roundtrip() = runTest {
    ...
    @Test
    fun currency_rate_roundtrip() = runTest {
        val fromId = db.currencyDao().upsert(
            CurrencyEntity(code = "USD", symbol = "$", name = "US Dollar", decimalDigits = 2, isActive = true, sortOrder = 0),
        )
        val toId = db.currencyDao().upsert(
            CurrencyEntity(code = "EUR", symbol = "€", name = "Euro", decimalDigits = 2, isActive = true, sortOrder = 1),
        )
        val id = db.currencyRateDao().upsert(
            CurrencyRateEntity(
