...
class TransferExecutor @Inject constructor(
    ...
    suspend fun execute(
        ...
        val (toAmount, exchangeRate) = if (sameCurrency) {
            amount to null
        } else {
            val rate = currencyRateRepository.findRate(source.currencyId, target.currencyId)
...
sealed class TransferResult {
    ...
    sealed class Failure : TransferResult() {
        data class SourceMissing(val accountId: Long) : Failure()
        data class TargetMissing(val accountId: Long) : Failure()
        data class RateMissing(val fromCurrencyId: Long, val toCurrencyId: Long) : Failure()
    }
}
