...
class GetTransferRecordsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        accountId: Long?,
        period: Period,
    ): List<TransferRecord> = withContext(defaultDispatcher) {
        val rows = transactionRepository.getTransfers(accountId, period)
        rows.map { row ->
            val currency = currencyRepository.findById(row.currencyId)
