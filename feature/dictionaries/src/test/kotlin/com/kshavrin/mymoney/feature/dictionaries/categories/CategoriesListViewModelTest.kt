...
class CategoriesListViewModelTest {
    ...
    @Test
    fun `state separates expense and income sorted by sortOrder`() = runTest {
    ...
    @Test
    fun `reorder expense persists new order with sortOrder 0 to n`() = runTest {
        seedDefaultExpenseAndIncome()
        viewModel = CategoriesListViewModel(fakeRepo)
        ...
        val persistedExpense = fakeRepo.observeAll().first()
    ...
    @Test
    fun `reorder income persists new order with sortOrder 0 to n`() = runTest {
        seedDefaultExpenseAndIncome()
        viewModel = CategoriesListViewModel(fakeRepo)
        ...
        val persistedIncome = fakeRepo.observeAll().first()
    ...
    @Test
    fun `reordering expense does not touch income sortOrder`() = runTest {
        seedDefaultExpenseAndIncome()
        viewModel = CategoriesListViewModel(fakeRepo)

        val incomeBefore = fakeRepo.observeAll().first()
            .filter { it.kind == CategoryKind.Income }
        ...
        val incomeAfter = fakeRepo.observeAll().first()
            .filter { it.kind == CategoryKind.Income }
            .associate { it.id to it.sortOrder }
    ...
    @Test
    fun `reordering income does not touch expense sortOrder`() = runTest {
        seedDefaultExpenseAndIncome()
        viewModel = CategoriesListViewModel(fakeRepo)

        val expenseBefore = fakeRepo.observeAll().first()
            .filter { it.kind == CategoryKind.Expense }
            .associate { it.id to it.sortOrder }
        ...
        val expenseAfter = fakeRepo.observeAll().first()
            .filter { it.kind == CategoryKind.Expense }
            .associate { it.id to it.sortOrder }
    ...
    @Test
    fun `reorder updates uiState expense immediately to new order`() = runTest {
        seedDefaultExpenseAndIncome()
        viewModel = CategoriesListViewModel(fakeRepo)
    ...
    @Test
    fun `reorder updates uiState income immediately to new order`() = runTest {
        seedDefaultExpenseAndIncome()
        viewModel = CategoriesListViewModel(fakeRepo)
    ...
    @Test
    fun `reorder preserves all non-sortOrder fields of expense items`() = runTest {
        seedDefaultExpenseAndIncome()
        viewModel = CategoriesListViewModel(fakeRepo)
        ...
        val persisted = fakeRepo.observeAll().first()
            .filter { it.kind == CategoryKind.Expense }
            .associateBy { it.id }
    ...
    @Test
    fun `reorder with single item assigns sortOrder zero`() = runTest {
        fakeRepo.seed(
    ...
    @Test
    fun `reorder with empty list does not crash and does not affect other section`() = runTest {
        fakeRepo.seed(
            category(10L, "Salary", CategoryKind.Income, sortOrder = 0),
            category(11L, "Gifts", CategoryKind.Income, sortOrder = 1),
        )
        ...
        val income = fakeRepo.observeAll().first()
            .filter { it.kind == CategoryKind.Income }
            .sortedBy { it.sortOrder }
