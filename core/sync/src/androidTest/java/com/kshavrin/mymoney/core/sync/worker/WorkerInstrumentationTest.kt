...
class WorkerInstrumentationTest {
    ...
    @Test
    fun recurringWorkerGeneratesDueOccurrenceSilently() = runBlocking {
        ...
        val worker = TestListenableWorkerBuilder<RecurringWorker>(context)
    ...
    @Test
    fun pruneDeletedWorkerRemovesOldDeletedButKeepsRecent() = runBlocking {
        seeder.seedIfNeeded(Instant.now())
        val cash = accountRepository.observeActive().first().first()
        val category = categoryRepository.observeAll().first().first { it.kind.name == "Expense" }
        val now = Instant.now()
        val oldId = transactionRepository.upsert(
            deletedTransaction(cash.id, category.id, cash.currencyId, updatedAt = now.minus(40, ChronoUnit.DAYS)),
        )
        val recentId = transactionRepository.upsert(
            deletedTransaction(cash.id, category.id, cash.currencyId, updatedAt = now),
        )

        val worker = TestListenableWorkerBuilder<PruneDeletedWorker>(context)
            .setWorkerFactory(workerFactory)
            .build()
        assertEquals(ListenableWorker.Result.success(), worker.doWork())
    ...
    @Test
    fun syncWorkerIsNoOpSuccessWhenAutoSyncDisabled() = runBlocking {
    ...
    @Test
    fun backupRotationWorkerFailsWhenTreeUriMissing() = runBlocking {
        val worker = TestListenableWorkerBuilder<BackupRotationWorker>(context)
            .setWorkerFactory(workerFactory)
            .build()
